...
got10k = json.load(open('/home/syh/siamdw/data/GOT10k/got10k.json', 'r'))
...
val_path = os.path.join(save_dir, 'val.json')

json.dump(train, open(train_path, 'w'), indent=4, sort_keys=True)
json.dump(val, open(val_path, 'w'), indent=4, sort_keys=True)

print('Save train data to {}'.format(train_path))
print('Save val data to {}'.format(val_path))
